...
if(isset($_POST["delete-btn"])){
    mysqli_query($conn, "DELETE FROM `user_form` WHERE id = '$user_id'");
    session_destroy();
    header('location:login.php');
}
$order = mysqli_query($conn,"SELECT * FROM `order` WHERE user_id = '$user_id'");
?>
...
<body>
    ...
    <div id="box">
        ...
        <h1 id="titulo" >Perfil</h1>
            ...
            <div class="container">
                ...
                <p> Nome do usuário : <span><?php echo $fetch_user['name']; ?></span> </p>
                <p> E-mail : <span><?php echo $fetch_user['email']; ?></span> </p>

                <form name="cadastro" method="post" action="#">
                <input type="submit" value="Excluir conta" name="delete-btn" id="submit" onclick="return confirm('Você tem certeza que deseja excluir sua conta?');"/>
                </form>
    ...
    <h2 id="promoção">SEUS PEDIDOS:</h1><br>
                <?php
                    echo "<table border='1vw'>";
                    echo "<tr class='titulo'><th>Nome</th><th>Número</th><th>Email</th><th>Método</th><th>Endereço</th><th>Produtos</th><th>Preço Final</th><tr>";
                
                    while($dados=mysqli_fetch_array($order))
                    {
                        echo "<tr>";
                        echo "<td>". $dados['name']."</td>";
                        echo "<td>". $dados['number']."</td>";
                        echo "<td>". $dados['email']."</td>";
                        echo "<td>". $dados['method']."</td>";
                        echo "<td>". $dados['address']."</td>";
                        echo "<td>". $dados['total_products']."</td>";
                        echo "<td>". $dados['total_price']."</td>";
                        echo "</tr>";
                    }
                    echo "</table>";
                ?>
