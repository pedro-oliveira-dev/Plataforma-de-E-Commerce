...
$product_review_id = base64_decode($_GET['id']);

$select_product = mysqli_query($conn, "SELECT * FROM `products` where id = $product_review_id") or die('query failed');
...
if(isset($_POST['submit'])){
    ...
    $verify_review = mysqli_query($conn, "SELECT * FROM `reviews` WHERE product_id = $product_review_id AND user_id = $user_id") or die('query failed');
 
    if(mysqli_num_rows($verify_review) > 0){
       $message[] = 'Você já havia dado uma avaliação antes!';
    }else{
       mysqli_query($conn, "INSERT INTO `reviews`(id, product_id, user_id, name, rating, title, description, question1, question2, question3, question4, question5, question6) VALUES('$id', '$product_review_id', '$user_id','$name','$rating','$title','$description','$question1','$question2','$question3','$question4','$question5','$question6')") or die('query failed');
       if($rating == 'like'){
            mysqli_query($conn, "UPDATE `products` SET `like` = `like` + 1 WHERE `id` = $product_review_id") or die('query failed');
       }else{
            mysqli_query($conn, "UPDATE `products` SET `deslike` = `deslike` + 1 WHERE `id` = $product_review_id") or die('query failed');
       }
       $message[] = 'O comentário foi postado';
    }
...
?>
    ...
    <div id="box">
        ...
        <h1 id="titulo" >Poste sua avaliação!</h>
        
                    <div id="cadastro">
                    <form action="" method="post">
                        <label for="title"><b>Título:</b></label><br><br>
                        <input type="text" id="fname" name="title" required placeholder="Insira o título."><br><br>
                        <label for="description"><b>Descrição:</b></label><br><br>
                        <textarea id="email" name="description" required placeholder="Escreva sua avaliação aqui!" maxlength="1000" cols="30" rows="10"></textarea><br><br>
                    <?php 
                            if($fetch_product['category'] == "pc"){
                    ...
                    ?>
                        ...
                        <select id="method" name="rating">
                            <option value="like" selected>Gostei</option>
