...
$cart_query = mysqli_query($conn, "SELECT * FROM `order` WHERE user_id = $user_id");
if(mysqli_num_rows($cart_query) == 0){
    header('location:index.php');
 }else{
    $order = mysqli_query($conn, "SELECT * FROM `order` WHERE user_id = $user_id AND id = (SELECT MAX(id) from `order`)");
    $fetch_order = mysqli_fetch_assoc($order);
 }
?>
...
<head>
    ...
    <link rel="stylesheet" href="../estilos/concluir.css" />
    ...
    <div class='order-message-container'>
        <div class='message-container'>
            ...
            <div class='order-detail'>
                <span><?php echo $fetch_order['total_products']; ?></span>
                <br><br>
                <?php //formatando o número de preço para o formato br
                    $preco = $fetch_order['total_price'];
                    $preco = number_format($preco, 2, ',', '.');
                ?>
                <span class='total'> Total R$: <p id="dinheiro"><?php echo $preco; ?></p> </span>
            </div>
            <br>
            <div class='customer-details'>
                <p> Seu nome : <?php echo $fetch_order['name']; ?></p>
                <p> Seu numero : <?php echo $fetch_order['number']; ?></p>
                <p> Seu email : <?php echo $fetch_order['email']; ?></p>
                <p> Seu endereço : <?php echo $fetch_order['address']; ?></p>
                <p> Modo de Pagamento : <?php echo $fetch_order['method']; ?></p>
                <br> <br>
            <div id="botao">
                <a id="botao1" href='index.php' class='btn'>Continue Comprando</a>
            </div>
            <br>
    </div>
