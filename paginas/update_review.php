...
$rating_review = mysqli_query($conn, "SELECT * FROM `reviews` WHERE id = '$id' LIMIT 1") or die('query failed');
if(mysqli_num_rows($rating_review) > 0){
    $fetch_rating = mysqli_fetch_assoc($rating_review);
    $product_id = $fetch_rating['product_id']; 
    $old_rating = $fetch_rating['rating'];
}
...
if(isset($_POST['submit'])){
    ...
    if($rating != $old_rating){
        ...
        if($counter > 0){
            if($rating == 'like'){
                mysqli_query($conn, "UPDATE `products` SET `like` = `like` + 1, `deslike` = `deslike` - 1 WHERE `id` = $product_id") or die('query failed');
            }else{
                mysqli_query($conn, "UPDATE `products` SET `deslike` = `deslike` + 1, `like` = `like` - 1 WHERE `id` = $product_id") or die('query failed');
            }
        }
    }

    $update_review = mysqli_query($conn, "UPDATE `reviews` SET rating = '$rating', title = '$title', description = '$description', question1 = '$question1', question2 = '$question2', question3 = '$question3', question4 = '$question4', question5 = '$question5', question6 = '$question6' WHERE id = $id") or die('query failed');
...
?>
    ...
    <h1 id="titulo" >Edite sua avaliação!</h>

                <div id="cadastro">
                <?php
                    $select_review = mysqli_query($conn, "SELECT * FROM `reviews` WHERE id = '$id' LIMIT 1") or die('query failed');
                    if(mysqli_num_rows($select_review) > 0){
                        while($fetch_review = mysqli_fetch_assoc($select_review)){
                ?>
                <form action="" method="post">
                    <label for="title"><b>Título:</b></label><br><br>
                    <input type="text" id="fname" name="title" required placeholder="Insira o título." value="<?= $fetch_review['title']; ?>"><br><br>
                    <label for="description"><b>Descrição:</b></label><br><br>
                    <textarea id="email" name="description" required placeholder="Escreva sua avaliação aqui!" maxlength="1000" cols="30" rows="10"><?= $fetch_review['description']; ?></textarea><br><br>

                    <?php 
                            if($fetch_product['category'] == "pc"){
                                echo'<label for="question1"><b>O computador é voltado para ser:</b></label><br><br>
                                <select id="method" name="question1">
                                    <option value="Gamer" selected>Gamer</option>
                                    <option value="Casa e escritório">Casa e escritório</option>
                                    <option value="Workstation">Workstation</option>
                                </select><br><br>

                                <label for="question2"><b>Armazenamento é suficiente?</b></label><br><br>
                                <select id="method" name="question2">
                                    <option value="Sim" selected>Sim</option>
                                    <option value="Não">Não</option>
                                </select><br><br>

                                <label for="question3"><b>Entregou um desempenho esperado?</b></label><br><br>
                                <select id="method" name="question3">
                                    <option value="Desempenho ótimo" selected>Desempenho ótimo</option>
                                    <option value="Desempenho aceitável">Desempenho aceitável</option>
                                    <option value="Desempenho ruim">Desempenho ruim</option>
                                </select><br><br>

                                <label for="question4"><b>É possível fazer tarefas simples sem travamentos?</b></label><br><br>
                                <select id="method" name="question4">
                                    <option value="Sim" selected>Sim</option>
                                    <option value="Não">Não</option>
                                </select><br><br>

                                <label for="question5"><b>Nos jogos testados, a taxa de quadros foi igual/superior a 60 quadros?</b></label><br><br>
                                <select id="method" name="question5">
                                    <option value="Sim" selected>Sim</option>
                                    <option value="Não">Não</option>
                                    <option value="Não sei informar">Não sei informar</option>
                                </select><br><br>

                                <label for="question6"><b>O computador possui um resfriamento adequado?</b></label><br><br>
                                <select id="method" name="question6">
                                    <option value="Sim" selected>Sim</option>
                                    <option value="Não">Não</option>
                                    <option value="Não sei informar">Não sei informar</option>
                                </select><br><br>';
                    ...
                    <label for="rating"><b>Avaliação Final:</b></label><br><br>
                    <select id="method" name="rating">
                        <option value="<?= $fetch_review['rating']; ?>" selected>Opinião final anterior: <?php if ($fetch_review['rating'] == 'like') {echo 'Gostei';} else{ echo 'Não Gostei';} ?></option>
                        <option value="like">Gostei</option>
                        <option value="deslike">Não Gostei</option>
                    </select><br><br>
                    
                    <div id='botão'>
                        <input type="submit" name="submit" value="Postar" id="confirmar"/>
                    </div>
                </form>
                </div>
                <?php
                        ...
                        echo '<p class="empty">Algo deu errado!</p>';
                    }
                ?>
    </div> <br/><br/>
